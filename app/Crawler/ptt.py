...
if not os.path.isdir('history'):#如果沒有資料夾，就建立資料夾
    os.mkdir('history')
...
def crawler(table_name,page): #主要程式
...
def ETL(df,column):
    ...
    for index,word in enumerate(removeword):
...
def new_page(table_name): #用於取得最新頁面
    ...
    # try:
    #     driver = webdriver.Chrome(os.path.join('engine',"chromedriver_106.0.5249.61.exe")) 
    # except:
